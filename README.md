...
- Message and token fields are left blank when Claude does not expose reliable values for them.
...
Claude does not document a public browser API for usage limits. This extension reads Claude's usage endpoint in the background, falls back to visible text on `https://claude.ai/settings/usage`, and keeps that synced value cached locally. It does not render fake usage percentages, message counts, or token estimates.
